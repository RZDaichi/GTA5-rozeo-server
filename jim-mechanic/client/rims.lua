...
RegisterNetEvent('jim-mechanic:client:Rims:Apply', function(data)
	...
	{}, {}, function()
		...
		updateCar(vehicle)
		if data.mod == -1 then TriggerEvent('jim-mechanic:client:Rims:Choose', data)
...
RegisterNetEvent('jim-mechanic:client:Rims:Choose', function(data)
	...
	if not IsPedInAnyVehicle(playerPed, false) then vehicle = getClosest(coords) pushVehicle(vehicle)
		if DoesEntityExist(vehicle) then
			...
			local RimsMenu = {
					{ isMenuHeader = true, header = searchCar(vehicle).." "..Loc[Config.Lan]["rims"].menuheader.."<br>("..string.upper(label)..")", txt = "", },
				...
				for k, v in pairs(validMods) do
					if not validMods[k][1] then
						RimsMenu[#RimsMenu + 1] = { header = v.name, txt = v.install, params = { event = 'jim-mechanic:client:Rims:Apply', args = { mod = v.id, wheeltype = data.wheeltype, wheelname = GetLabelText(GetModTextLabel(vehicle, 23, GetVehicleMod(vehicle, 23))) } } }
					else
						if k == "NULL" then setheader = Loc[Config.Lan]["rims"].labelcustom else setheader = k end
						RimsMenu[#RimsMenu + 1] = { header = setheader, txt = Loc[Config.Lan]["common"].amountoption..#validMods[k], params = { event = 'jim-mechanic:client:Rims:SubMenu', args = { mod = v.id, wheeltype = data.wheeltype, wheeltable = validMods[k], bike = data.bike } } }
					end
				end
	...
	if not IsPedInAnyVehicle(playerPed, false) then vehicle = getClosest(coords) pushVehicle(vehicle)
		if DoesEntityExist(vehicle) then
			local RimsMenu = {
					...
					{ header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "jim-mechanic:client:Rims:Choose", args = { wheeltype = data.wheeltype } } } }
