...
RegisterNetEvent('jim-mechanic:client:Bumpers:Check', function()
	...
	if DoesEntityExist(vehicle) then
		...
		if numMods1 ~= 0 then BumperMenu[#BumperMenu + 1] = { header = Loc[Config.Lan]["bumpers"].menugrille, txt = "["..(GetNumVehicleMods(vehicle, 6)+1)..Loc[Config.Lan]["common"].menuinstalled..installed1, params = { event = "jim-mechanic:client:Bumpers:Choose", args = 6 } } end
		if numMods2 ~= 0 then BumperMenu[#BumperMenu + 1] = { header = Loc[Config.Lan]["bumpers"].menuFBumper, txt = "["..(GetNumVehicleMods(vehicle, 1)+1)..Loc[Config.Lan]["common"].menuinstalled..installed2, params = { event = "jim-mechanic:client:Bumpers:Choose", args = 1 } } end
		if numMods3 ~= 0 then BumperMenu[#BumperMenu + 1] = { header = Loc[Config.Lan]["bumpers"].menuBBumper, txt = "["..(GetNumVehicleMods(vehicle, 2)+1)..Loc[Config.Lan]["common"].menuinstalled..installed3, params = { event = "jim-mechanic:client:Bumpers:Choose", args = 2 } } end
