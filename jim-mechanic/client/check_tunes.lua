...
RegisterNetEvent('jim-mechanic:client:Menu', function()
	...
	local CheckMenu = {
		...
		if IsVehicleOwned(trim(GetVehicleNumberPlateText(vehicle))) then local VehicleNitrous = nil while VehicleNitrous == nil do QBCore.Functions.TriggerCallback('jim-mechanic:GetNosLoaded', function(vehs) VehicleNitrous = vehs end, trim(GetVehicleNumberPlateText(vehicle))) Wait(100) end
			if VehicleNitrous[GetVehicleNumberPlateText(vehicle)] == nil then else
				CheckMenu[#CheckMenu + 1] = { header = Loc[Config.Lan]["check"].label58, txt = "<img src=nui://"..Config.img..QBCore.Shared.Items["nos"].image.." width=13px> LVL: "..VehicleNitrous[GetVehicleNumberPlateText(vehicle)].level.."%", params = { event = "jim-mechanic:client:Menu:Remove", args = { vehicle = vehicle, mod = "nos" } } }
			end
		end
